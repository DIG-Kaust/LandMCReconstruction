...
from pylops.basicoperators import Diagonal, FirstDerivative, Identity
...
def multicomponent_slopes_inverse(data, dx, dt, graddata=None, Rop=None, reg=1e-1, use_weighted=True, **kwargs_solver):
    ...
    Lop = pylops.Laplacian((nx, nt),kind="centered")
    ...
    if use_weighted:
        ...
        slope_mc = pylops.optimization.leastsquares.regularized_inversion(
            Wop @ Ddt, -(Wop @ data_dx).ravel(), [Lop], epsRs=[reg], **kwargs_solver)[0]
    else:
        Wop = Identity((Rop@weight).shape[0])
        Wop = Diagonal(cp_asarray((Rop @ weight)).astype(np_floatconv))
        slope_mc = pylops.optimization.leastsquares.regularized_inversion(
